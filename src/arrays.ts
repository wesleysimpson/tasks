...
export function injectPositive(values: number[]): number[] {
    const firstNegativeIndex = values.findIndex((v: number): boolean => v < 0);

    const sumAll = values.reduce(
        (total: number, v: number): number => total + v,
        0,
    );

    if (firstNegativeIndex === -1) {
        // no negatives: append sum of all
        return [...values, sumAll];
    }

    const sumBeforeNegative = values
        .slice(0, firstNegativeIndex)
        .reduce((total: number, v: number): number => total + v, 0);

    // Insert AFTER the first negative number
    return [
